...
class DanNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.maxPool1_0 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        ...
        # https://pytorch.org/docs/master/nn.html#batchnorm2d
        # num_features/channels, eps, momentum, affine, track_running_stats
        self.conv2_1 = nn.Conv2d(64, 64, 3, stride=1, padding=0)

        # https://pytorch.org/docs/master/nn.html#maxpool2d
        # kernel_size, stride, padding, dilation, return_indices, ceil_mode
        self.maxPool1_1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)

        self.conv3_1 = nn.Conv2d(64, 128, 3, stride=1, padding=0)

        self.conv4_1 = nn.Conv2d(128, 128, 3, stride=1, padding=0)

        self.maxPool2_1 = nn.MaxPool2d(2, stride=2, padding=0)

        self.conv5_1 = nn.Conv2d(128, 256, 3, stride=1, padding=0)
        self.conv6_1 = nn.Conv2d(256, 256, 3, stride=1, padding=0)
        self.maxPool3_1 = nn.MaxPool2d(2, stride=2, padding=0)

        self.conv7_1 = nn.Conv2d(256, 512, 3, stride=1, padding=0)
        self.conv8_1 = nn.Conv2d(512, 512, 3, stride=1, padding=0)
        self.maxPool4_1 = nn.MaxPool2d(2, stride=2, padding=0)

        self.conv3D_01 = nn.Conv2d(in_channels=1024, out_channels=1024, kernel_size=3, stride=1, padding=0)
        ...
        self.conv19 = nn.Conv2d(64, 1, 1, stride=1, padding=0)
    ...
    def forward(self, z, padding=False):
        ...
        batchsize = z.shape[0]
        x = z[:, 0, :, :].view(batchsize, 1,  z.shape[2],  z.shape[3])
        y = z[:, 1, :, :].view(batchsize, 1,  z.shape[2],  z.shape[3])
